...
function App() {
  ...
  const toggleval = async (id, val) => {
    try {
      ...
      setTodos((prev) =>
        prev.map((todos) => (todos.id === id ? res.data : todos))
      );
      if (val) {
        setInput("");
      }
    ...
    <div className="bg-gray-200 min-h-screen flex flex-col justify-center items-center">
      <div className="max-w-3xl bg-white p-6 rounded-xl w-full min-h-[500px] shadow-lg sm:p-8 text-center">
        ...
        <div className="mt-5 flex flex-col sm:flex-row items-center sm:items-start justify-around gap-8 sm:gap-4">
          <form
            onSubmit={addtodo}
            className="flex flex-col items-center justify-center gap-3 w-full sm:w-auto"
          >
            <label className="text-xl text-blue-400 font-[400]">
              Add New Todo
            </label>
            <input
              type="text"
            ...
            <textarea
              ...
              className="max-w-[200px] p-6 border border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-400"
              rows="3"
            ></textarea>{" "}
            {/* <--- ADDED w-full */}
                ...
                >
                  ...
                  <div className="flex flex-row gap-3 items-center justify-center mb-1 mt-1">
                    {!todo.completed ? (
                      ...
                      >
                        {!todo.completed ? "Completed" : "Congrats!"}
                      </button>
                    ) : (
                      <p
                        ref={congratsRef}
                        className="text-green-400 font-[600] text-[20px]"
                      >
                        Congrats!
                      </p>
                    )}
                    <button
                      ...
                      onClick={() => delete1(todo.id)}
                    >
                      <FaTrash></FaTrash>
                    </button>
                    <button
                      onClick={() => toggleval(todo.id, input, desc)}
                      className="px-3 py-1 bg-white rounded-lg text-black text-sm flex items-center gap-1"
                    >
                      <FaPencilAlt />
                    </button>
                  </div>
